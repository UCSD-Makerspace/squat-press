...
class LinearSensorThread(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            if self.recent_lifts is not None and len(self.recent_lifts) > 10:
            ...
            if mm_value is not None:
                ...
                if self.validate_lift(mm_value):
                    ...
                    while self.validate_lift(mm_value):
                        mm_value = self.linear_sensor.read_mm_value()
    ...
    def validate_lift(self, mm_value):
        if self.recent_lifts is None or len(self.recent_lifts) < 2:
            return False
        ...
        if (getattr(self, 'in_lift', False) and mm_value > (self.threshold) - 2):
            return True
